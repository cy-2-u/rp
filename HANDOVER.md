...
**部署只需要 5 个文件**：`_worker.js`、`magic-extension.js`、`DB/` 下全部 3 个（bootstrap.js、dirty-tracker.js、styles.css）。其余都不上传。
...
2. 在仓库里改完部署文件（`_worker.js`、`magic-extension.js`、`DB/*`，按需），commit 到 main 分支。
3. push 到 GitHub（cy-2-u/rp）：
   - 若 Pages 连的是这个仓库 → push 即自动部署；
   - 若是手动直传 → 在 Pages 的 Create deployment 里把 5 个文件按原目录结构拖进去。
4. 验证见第 7.3 节的两分钟自检。
...
```
cd <仓库clone目录>\.sync-tests    # 本机示例：D:\work\魔改版\.sync-tests
npm test
```

两个套件，全部离线（不碰 GitHub、不碰真实 R2、不碰真实数据）：
...
- 想测另一份清单：`RPHUB_ADAPTER_URL=file:///D:/某处/adapter.json npm test`。
...
- 删除墓碑（`rp-images/_deleted/`）随删除量缓慢增长且当前不做自动回收——墓碑同时承担"禁止重新生成已删图片"的语义；删除量极大时可由维护者手动批量清理。
- 同步面板 401 密码弹窗的 DOM 交互没有自动化用例（API 级 401 语义已覆盖）。
...
- `expectedMatches` 上限 8、`resolveAdapterUrl` 允许 http:// 等，都是刻意的宽松/收窄边界，改动前想清楚为什么。
