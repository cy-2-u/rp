...
async function fetchImageWithTimeout(url, options = {}) {
...
async function handleImageRender(request, env) {
    ...
    if (contentLength > IMAGE_MAX_BYTES) {
        return error(`\u56fe\u7247\u8fc7\u5927\uff1a${contentLength}/${IMAGE_MAX_BYTES}`, 413);
    }

    const bytes = await upstreamResponse.arrayBuffer();
    if (bytes.byteLength > IMAGE_MAX_BYTES) {
        return error(`\u56fe\u7247\u8fc7\u5927\uff1a${bytes.byteLength}/${IMAGE_MAX_BYTES}`, 413);
    }
...
async function readThumbnailBytes(request) {
    ...
    if (!bytes.byteLength || bytes.byteLength > IMAGE_THUMB_MAX_BYTES) {
...
async function handleImageAdmin(request, env, url, ctx) {
    ...
    if (url.pathname === `${IMAGE_ADMIN_PATH}/api/image`) {
        ...
        if (!key.startsWith(`${IMAGE_OBJECT_PREFIX}/`) || key.includes('..')) return error('\u56fe\u7247\u8def\u5f84\u65e0\u6548\u3002', 400);
